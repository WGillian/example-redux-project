...
const getSelectedAlcContentLower = (contentAmounts, selectedAmount) => {
  ...
  return indexOfSelectedAlcContent - 1 === -1 ? null : orderedAmountValues[indexOfSelectedAlcContent - 1]
}

const ALL_OPTION = { value: -1, label: 'All' }

export const punkSelector = createSelector(
  state => state.punk,
  punkState => {
    const alcoholContentOptions = [ALL_OPTION, ...punkState.alcoholContentOptions]
    const maltOptions = [ALL_OPTION, ...punkState.maltOptions]
    ...
    const fullSelectedAlcoholContent = _.find(alcoholContentOptions, option => option.value === punkState.selectedAlcoholContent)
    ...
    return {
      ...
      alcoholContentOptions: alcoholContentOptions,
      maltOptions: maltOptions,
    }
  },
)
